...
| **v2 — tidy** | Tidy up the transcript: a small language model takes out fillers and false starts beside the recogniser, every line checked word for word against what was spoken | Built 2026-09-02 and run beside the recogniser on one laptop the same day: nothing refused on 77 lines, the recogniser 31% slower for the company. Its delta on the ten-call WER corpus followed the same evening: +3.43 points against the verbatim transcripts, −2.94 against the edited ones, the whole difference the deletions the contract allows. The criterion reads the edited transcripts, so the first condition is met on one laptop. The unit one request carries was measured across three shapes on 2026-09-03: joined runs land the tidied copy 83 s after the plain transcript against the segment's 194 s and tidy better under both references, but a refused run refuses every line it carries, so the rule as written keeps the segment on that clause alone — how that clause should read is a decision now rather than a measurement. Not shipped: it still owes the desktop's CUDA delta and re-timing, that decision, and the retry's own measurement if the decision goes that way — [PHASES.md](docs/PHASES.md) |
