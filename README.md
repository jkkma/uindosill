...
| **Tidy up the transcript** | Opt-in, in the next release. A small language model (Gemma 4 E4B, 3.9 GiB) takes fillers, stutters and false starts out of the lines *beside* the recogniser, a few at a time, and every line is held to a delete-only contract against what was spoken: a rewrite that changes or adds a word is refused and the spoken line kept, the one exception a word the recogniser itself doubted. Deleting is what it is for, and since 2026-09-03 there is a bound on how much: a rewrite that drops more than half of a line, or more than four words in a row, is refused as well — without one the model lifted whole clauses out of sentences it read as stutters. The tidied version sits beside the transcript with the spoken words' timings, and never replaces it. Measured on one laptop and one desktop against eleven hours of human transcripts: 3.4 to 3.5 points worse against the style that writes every stutter down, 2.9 better against the edited style, and the same words are the reason for both. What it still owes is in [PHASES.md](docs/PHASES.md). |
