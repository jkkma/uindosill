...
| **v2 — tidy** | Tidy up the transcript: a small language model takes out fillers and false starts beside the recogniser, every line checked word for word against what was spoken | Built 2026-09-02 and run beside the recogniser on one laptop the same day: nothing refused on 77 lines, the recogniser 31% slower for the company. Not shipped: it still owes its WER-corpus delta and the desktop's CUDA re-timing — [PHASES.md](docs/PHASES.md) |
