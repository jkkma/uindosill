...
/// application's own settings. Both have to survive an update and an uninstall, and the way they
/// survive is by not being inside the install directory — see <c>docs/GOTCHAS.md</c> gotcha 8.
/// </para>
/// <para>
...
/// directory recursively, so a package id equal to the last segment here would make uninstall
/// delete every downloaded weight. <c>PackagingIdentity</c> holds the id that keeps them apart, and
