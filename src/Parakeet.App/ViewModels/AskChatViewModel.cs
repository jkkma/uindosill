    ...
    private async Task AskCore(string question)
    {
        ...
        try
        {
            ...
            if (evidence.Count == 0)
            {
                ...
                {
                    entry.Fail(
                        "This looks like a question about the whole recording, but there is "
                        + "nothing in this transcript to answer from.");
                }
                else
                {
...
public sealed partial class ChatEntryViewModel : ObservableObject
{
    ...
    private static string BuildModelLine(AnswerDocument answer)
    {
        ...
        // these are three different claims about coverage, and a survey is neither of the other
        // two — it read a little of all of it, where retrieval reads all of a little.
