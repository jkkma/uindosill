...
internal static class Commands
{
    ...
    public static readonly CommandSpec Transcribe = new()
    {
        ...
        [
            ...
            new OptionSpec
            {
                ...
                TakesValue = true,
                ValueName = "id",
                Help = "Catalogue id of the diarisation model. Default: the installed one. One entry labels " +
                       "speakers; the resolution is over what is installed rather than over the catalogue, so a " +
                       "machine with a second one is asked rather than chosen for.",
            },
            new OptionSpec
            {
                ...
                TakesValue = true,
                ValueName = "dir",
                Help = "Use a diarisation model directory directly instead of a catalogue entry. It is the pyannote " +
                       "pipeline's own layout, config.yaml included, because the pipeline resolves its parts " +
                       "through that file.",
            },
            new OptionSpec
            {
    ...
    public static readonly CommandSpec Diarise = new()
    {
        ...
        [
            ...
            new OptionSpec
            {
                ...
                TakesValue = true,
                ValueName = "dir",
                Help = "Use a model directory directly instead of a catalogue entry. It is the pyannote pipeline's " +
                       "own layout, config.yaml included, because the pipeline resolves its parts through that " +
                       "file.",
            },
            new OptionSpec
            {
            ...
            new OptionSpec
            {
                ...
                Help = "How many voices there are, when known. The diariser estimates the count and cannot be told it, " +
                       ...
                       "that talk over each other least. Nothing merges if it already found this many or fewer.",
            },
            Fake,
            Help,
        ],
        Details =
            ...
            "of appearance: it is the label the model itself assigned, and a scorer wants to see what the\n" +
            "model produced rather than a renaming of it.\n\n" +
    ...
    public static readonly CommandSpec Retrieve = new()
    {
        Name = "retrieve",
        Summary = "Search a transcript the way the Ask panel does: ranked windows with citation ids.",
        Positionals = "<transcript.json>",
        Options =
        [
            new OptionSpec
            {
                ...
                Repeatable = true,
                ValueName = "text",
                Help = "A question to search for. Repeatable; results come back in the same order.",
            },
            new OptionSpec
            {
                Name = "top",
            ...
            {
                Name = "wide",
                Help = "The 120 s window variant docs/V2-ASK-THE-TRANSCRIPT.md's decision 3 names for " +
                       "comparison runs, instead of the ~60 s default the panel uses.",
            },
            new OptionSpec
            {
                Name = "json",
                Help = "Machine-readable output: per question, the ranked windows with citation ids, segment " +
                       "ranges, times and scores.",
            },
            Help,
        ],
        Details =
    ...
    public static IReadOnlyList<CommandSpec> All { get; } =
        [Transcribe, Diarise, Translate, Models, Bench, Doctor, Probe, Notice, Formats, Wer, Der, Rttm, Retrieve];
}
