...
class MelFeaturizer:
    def __init__(self):
        ...
        # **The filterbank is a committed matrix, not a `librosa.filters.mel` call.**
        # Until 2026-08-26 this line was that call, and this file was the spike's byte for byte
        # because that fidelity is what makes the measured 16.3324% AMI figure describe this code.
        # The matrix below *is* that call's output, generated by librosa 1.0.0 -- the pinned
        # version the figure was produced on -- at exactly the parameters above, and committed.
        #
        # **Replacing the call with its result was measured before it was made**, on three
        # counts: the matrix is deterministic across calls, it round-trips through `.npy`
        # bit-for-bit, and the mel features this class produces on sixty seconds of real audio
        # are *bit-identical* either way. So the figure still describes this code -- and describes
        # it more tightly than before, because a committed array cannot drift where a library
        # call can. What changed is which of the two this file depends on, not what it computes.
        #
        # What it buys is not tidiness. librosa was the whole reason this bundle carried numba,
        # llvmlite and about 330 MB of transitive dependencies, and the reason it carried `soxr`,
        # whose libsoxr is statically linked and was the only LGPL binary here that LGPL-2.1
        # section 6(b) could not cover. `docs/LICENSING.md` records that reading.
        fb = np.load(os.path.join(os.path.dirname(__file__), "mel-filterbank.npy"))
        if fb.shape != (N_MELS, N_FFT // 2 + 1):
            # A fixture that does not match the geometry above would otherwise fail much later,
            # inside a matmul, as a shape error about numbers nobody can trace back to here.
            raise ValueError(
                f"mel-filterbank.npy is {fb.shape}, not {(N_MELS, N_FFT // 2 + 1)} as this file's "
                "geometry requires; it and these constants have gone out of step."
            )
