...
| The bundled Python packages (pinned in `python/requirements-bundle.txt`) | Mostly MIT, BSD and Apache-2.0 — but `soxr` is LGPL-2.1-or-later and `soundfile`'s wheel carries LGPL-2.1 `libsndfile`. **All 108 are listed below**, generated from an assembled bundle. The LGPL pair are not alike: libsndfile is a separate DLL loaded at run time and replaceable, libsoxr is statically linked into `soxr/soxr_ext.pyd`. `docs/LICENSING.md` reads what each obliges and records the obligation as live and not yet discharged | https://pypi.org/ |
...
a CPython 3.12.10 and the wheels `python/requirements-bundle.txt` pins — **108 distributions** once
