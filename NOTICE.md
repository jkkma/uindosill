...
a CPython 3.12.10 and the wheels `python/requirements-bundle.txt` pins — **99 distributions** once
...
> **The table below is stale as of 2026-08-27 and has not yet been regenerated.** The second
> diariser moved from DiariZen's vendored fork of pyannote-audio 3.1.1 to the released
> `pyannote.audio==4.0.7` wheel that day, which replaces the four pinned `pyannote.*` rows here —
> `pyannote.core` 5.0.0, `pyannote.database` 5.1.3, `pyannote.metrics` 3.2.1, `pyannote.pipeline`
> 3.0.1 — with that wheel's own dependency closure at its own floors, and adds `lightning`,
> `matplotlib`, `rich`, three `opentelemetry` packages, `torch-audiomentations`,
> `pytorch-metric-learning`, `asteroid-filterbanks`, `pyannoteai-sdk` and `torchcodec`. **The "99
> distributions" count above is therefore a floor rather than the number.** Neither the count nor
> the rows can be corrected from a manifest: they are read off an assembled bundle, and one has not
> been built since the swap. **Run `scripts/bundle-python.ps1` and then
> `scripts/collect-python-notices.py` before the next release.** `docs/UNPROVEN.md` carries the gap.
>
> None of the additions is expected to change the licensing conclusion above — `torchcodec` is the
> one worth naming, and it redistributes no FFmpeg: its own description says it uses whatever FFmpeg
> is already installed, so the LGPL exit of 1702d9e stands. That is a reading of upstream's
> documentation, not of an assembled wheel.
