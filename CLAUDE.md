...
dotnet test  Uindosill.slnx -c Release   # 913 tests, no weights, no display, no network
...
**Two of those 874 skip themselves.** One is the Media Foundation extension list, which is
...
`scripts/lab.ps1` is one entry point for the fifteen scripts; run it bare to list them.
