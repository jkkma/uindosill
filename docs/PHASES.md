...
| v2 — tidy | The tidy ships when its delta on the ten-call WER corpus has been measured under both reference styles — composition included, since the contract allows deletions and forbids everything but doubted words — and the desktop has re-timed the pass and its tandem on CUDA | **Decided 2026-09-01, not built.** Measured first, on the second machine: `gemma-4-E4B-it` tidies at 3.7 min per hour of audio with four requests in flight, saves 26% of the combined time run beside the recogniser on a ten-minute file at the cost of a 27% slower transcription, and before any contract changes one word in ~300 — `docs/UNPROVEN.md`, *Gemma 4 E4B as a transcript tidy* |
...
time? Measured before it was reasoned about, on the second machine, through the vendored
`llama-server` at the engine's own flags, with the E4B at the same QAT UD-Q4_K_XL quantisation the
...
realtime on this adapter (RTF 0.032–0.038 across the files measured), so a tidy run after it
...
(*objec* → *object*), guesses that may not be (*hair about* → *worry about*, a word invented), and
two that changed meaning. A three-hour recording would carry a hundred such edits with nothing to
...
27% slower for the company and the E4B at 61% of its solo rate. Timed end to end on the ten-minute
...
- **A tidied line is accepted only when its words are a subsequence of the spoken line's**, under
  ...
  On the 200 lines measured, about fifteen would have been refused; the rest pass.
...
with the measured trade in front of it: 43.1 s against 58.4 s on the ten-minute sample, a 26%
saving of the combined time, bought with a transcription 27% slower the whole way and a tidied
pane that trails further behind the longer the file runs. The alternative on the table was the
...
**The window shows the spoken line at once and swaps in the tidied one when it lands**, one to
four seconds later on the second machine, marked until then so a reader can tell a raw line from
...
tidying with whatever answering model is installed, nothing to download and 2.9–3.8 tok/s where
