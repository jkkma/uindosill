...
**Status:** met. 1882 tests, no weights, no display, no network — **1873 passed and 9 skipped**, and
...
**Status:** usable, tested end to end against the canned engine (137 of the project's 206 CLI
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** `AGENTS.md`'s second
count said 949 and had been stale by thirty for some time, because `949 skip` does not match the
pattern `scripts/check-test-counts.py` looks for; it is reworded to `1882 tests` so the guard now
covers it.
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.**
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Two of the four
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** The C# suite does
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Twenty-two new, and
...
page instead. **1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** The
...
assertion. **1882 tests, 1873 passed and 9 skipped, the count unchanged**: the file-writing
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Six of the new ones
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** No test changed: the
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped**, up from 1144;
...
retrieval as success, and the three refusals. **1882 tests, no weights, no display, no network —
1873 passed and 9 skipped**, up from 1262; 206 CLI tests.
...
the file's SHA-256 is unchanged across a full suite that previously rewrote it. **1882 tests, no
weights, no display, no network — 1873 passed and 9 skipped.** `docs/GOTCHAS.md` gotcha 33
carries the shape.
...
`docs/UNPROVEN.md`. **1882 tests, 1873 passed and 9 skipped, 0 warnings**, and the gated engine
...
**1882 tests, 1873 passed and 9 skipped, 0 warnings**, the gated engine trio green on cpu and
...
change, and the count this document quotes is the current one — **1882 tests, 1873 passed and 9
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** One new, the twin of
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** None new: the
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** None new, and one
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Seven new, all on
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Four new, all on the
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Eighteen new: twelve
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Ten new, all on the
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** None added and four
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Two new and one
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** None added; two
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** Six new, on the one
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** Thirteen new: seven
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** One new, on the
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** None added; three
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** None added: the
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** Two new: one that
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** Two new — the
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** Sixteen new
...
**1882 tests, no weights, no display, no network - 1873 passed and 9 skipped.** Three new: that the
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Six new: the
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** One new — a copy
...
end-to-end tests drive the options through the entry point; the suite is **1882 tests, 1873
...
### Found and fixed 2026-09-03, later — the fraction ceiling counted a stutter as a loss, and a repetition is now transparent to it

Found the same way as the defect it corrects, and hours after it: the maintainer driving the
shipped window, reporting that the tidy kept repetitions like *I I I I* and *y y you*. No harness,
no corpus, no `runs/` record, and this time no drive of the CLI either — the diagnosis is unit
arithmetic over `TidyContract`, which is pure logic and needs no model.

**What was wrong.** `DeletionRefusal` compares the dropped content words of a line against
`content * MaxDeletedFraction`, and a stammered line is by definition mostly stutter. Against the
ceilings as they shipped that morning:

| line | rewrite | outcome |
| --- | --- | --- |
| `I I I I think we should go` | `I think we should go` | accepted |
| `I I I I think` | `I think` | **refused** — dropped 3 of 5 |
| `y y you know what I mean` | `you know what I mean` | accepted |
| `y y you` | `you` | **refused** — dropped 2 of 3 |

The rule ran backwards: the denser the disfluency the more certain it survived, so a line that was
mostly stutter — the line most in need of tidying — was the one guaranteed to keep it. And because
a piece past either ceiling refuses its unit, one short stuttery segment took up to fifteen seconds
of well-tidied neighbours back to spoken with it.

**The fraction was never the net that caught the clause.** The entry above records, of the one call
both ceilings were fitted on, a legitimate stutter cleanup at 43% of its line against the clause
deletion at 33% — the bad edit scored *lower* on the proportion than the good one, and only
contiguity separated them. The fraction's own remark says it catches "a short line losing most of
itself"; a stutter collapse is exactly that shape while losing nothing, because the words it drops
are the word it keeps.

**The fix.** A dropped word is transparent to the fraction when it repeats the nearest word the
rewrite kept on either side, or is a false start of it — a proper prefix, which is what `y y you`
needs. Both directions are asked because which copy of a stutter the alignment keeps is its own
business: the first in `I I I I think`, the last in `y y you`. Not chosen: raising
`MaxDeletedFraction`, or dropping it, which would have been a number fitted to the examples that
prompted the complaint — the same objection this record already makes to 0.5 and 4 — where the
proportion is simply the wrong statistic for the thing it was being asked to judge.

**Deliberately narrow.** `MaxConsecutiveDeletedWords` is untouched and still counts every dropped
word, repetition or not, so the clause and the sentence of the entry above are refused by it
exactly as before — neither repeats what surrounds it, both run to five or more in a row, and the
two tests pinning them pass unchanged. `CountDeleted` is untouched, so the `DeletedWords` a
transcript reports still counts what actually went; only the fraction's numerator sees the
exemption. Both ceilings keep their shipped values, so nothing here was retuned.

Two tests, both on the contract: the two stutter shapes accepted under the shipped 0.5 and 4 with
`DeletedWords` still counting three and two, and a short line losing words that repeat nothing
still refused — the failure the ceiling exists for, on a line the run ceiling would not catch. The
suite moves 1633 to 1635.

**Nothing was measured.** The reported symptom is gone on the file that prompted it, observed in
the window and not scored against anything; there is no reference transcript, no WER or chrF
...
and the Japanese timings. The shipped `Helsinki-NLP/opus-mt-tc-bible-big-mul-deu_eng_nld` — the Hub
checkpoint behind the catalogue's `opus-mt-tc-bible-big-mul-en` directory — does contain `jpn` and would
...
between a user and that. **It was measured here that evening**: chrF++ 46.47 on FLEURS `ja_jp`, a
gate pass 6.06 below FuguMT on the same 321 sentences (*Measured 2026-09-04, evening*). That says
nothing about the 4.8 — a chrF++ on FLEURS cannot judge a BLEU on newstest2021 — but it does mean
**the two published figures did not predict this project's own result**, and the margin the choice
buys is now a measured one rather than an inferred one. The choice below stands; it was made on
architecture and licensing as much as on score.
fugumt would be the catalogue's first CC-BY-SA entry, which
...
The Japanese entry shipped earlier the same day with two rough edges named in its own catalogue
notes. This is the first of them. `docs/UNPROVEN.md` records what it did not fix.

**`CjkLineBreaking`** is a small implementation of a large convention: it recognises the scripts
written without spaces between words, measures a line in columns rather than characters — two per
full-width character, one per half-width, which is the same arithmetic as the guides' "half-width
counts as 0.5" — and breaks between characters under **kinsoku shori**, so no line opens with 。 or
、 or a closing bracket or a small kana, and none closes with an opening bracket. It enumerates
`Rune`, because breaking between the surrogates of a non-BMP kanji puts two replacement characters
on screen, and the surname kanji live up there.

`SubtitleOptions.MaxFullWidthCharactersPerLine` is **13**, which is Netflix's Japanese Timed Text
Style Guide and is exposed as a setting precisely because it is somebody's house style — no NHK or
ARIB guideline was obtained, and this project has measured nothing about Japanese readability.

**Every behaviour change is gated on the text.** Text with no full-width character cannot reach any
of it and takes the path it always took; the seven hundred formatting tests either side of the new
ones pass unchanged, which is the guard that matters more than any assertion written for this.

**A word that is the whole sentence now goes down the proportional path**, and that is the part
worth reading twice. The word path exists to cut cues at word boundaries and hand the word-timed
writer a word per span. On Japanese `group_words` finds no boundary at all — 30 of that model's
3,072 pieces carry a meta-space — so a "word" is a segment-level timing wearing a word's name, and
feeding it to the word path yielded one cue, one line, one span over eleven seconds. It now takes
the route this project already had for exactly this situation: split by capacity, time each piece by
its share of the characters, and emit **no** word timings rather than character-share timings
dressed up as measured ones. `vtt-words` renders such a cue as plain text, so **stripping the tags
still reproduces the plain `vtt` byte for byte** — the invariant that whole format rests on, and a
test now holds it on Japanese as well as English.

Measured on the sentence that prompted it: 74 characters that were one unwrapped line over 11.52 s
are now three cues of two 13-character lines.

Eleven tests. The suite moves 1656 to 1667.

**What it does not fix, and the notes say so to the user**: the reading rate — those cues run about
5.8 characters a second against the guide's 4, because nothing splits a cue for being too fast — and
the break landing mid-word (カタ / ルーニャ), where a subtitler would break at a phrase boundary,
which needs a morphological analyser this product does not carry. Word-timed VTT still cannot follow
a Japanese line, and now says so by carrying no timings rather than one span.

### Built 2026-09-04, later still — the Japanese reading rate, and the arithmetic that says what it can be

The second of the two rough edges the Japanese entry shipped with. The work is small; the finding
underneath it is the point.

**The rate cannot be fixed by cutting the text differently, and this is provable rather than
arguable.** A cue's duration is its share of the segment's characters, so every cue in a segment
already runs at the segment's own rate — the partition is irrelevant, and character-share timing is
already the arrangement that minimises the peak. Netflix's guide asks for 4 full-width characters a
second; the sentence that prompted this delivers 74 characters in 12.84 seconds, which is 5.8.
**That is a fact about how fast the person spoke.** Subtitlers meet a rate limit by condensing the
words, and this product must not: the transcript is what was said.

So `MaxFullWidthCharactersPerSecond` (4.0, Netflix's number again, a setting for the same reason
the line length is) does exactly one thing. `Tidy` already extended a cue to a readability floor and
already clamped it off the next cue's start; the floor is now the larger of that minimum and the
time the text needs to be read at this rate. Where a cue is followed by **silence** it takes what it
needs of it. Where more speech follows immediately there is nothing to take and the cue does not
move.

On the sentence above: cues 1 and 2 are unchanged at 4.450 s each, because speech follows them
immediately; cue 3 runs to **14.652 s instead of 12.840**, holding into the silence at the end.
**On continuous speech the rule does nothing at all**, which is the honest outcome and is what the
option's own documentation says before a reader can be misled by its name.

Latin is untouched — English subtitling has its own reading-rate conventions, this project has never
set one, and `ReadingTimeFor` returns zero for text with no full-width character in it, so every
existing cue keeps the time it had.

Five tests, and the one that matters is the negative: a cue followed immediately by more speech
keeps the duration the speech gave it and is never pushed over the next cue's start.
The suite moves 1667 to 1672.

**Still owed on Japanese subtitles**: the break lands mid-word — カタ / ルーニャ — where a subtitler
...
characters and 25,818 respectively, since stripping punctuation changes the denominator as well as
the errors, scored by `uindosill wer --cer` — the metric built the same morning, used rather than
...
so "FuguMT is better for Japanese" still rests on Helsinki's published figures — **scored that
evening at chrF++ 46.47, which passes the gate** (*Measured 2026-09-04, evening*), so the claim is
this harness's now and is narrower than the published gap. The cascade above remains FuguMT's alone;
...
### Measured 2026-09-04, evening — the shipped translator passes the gate on Japanese at chrF++ 46.47, and the published gap that chose FuguMT is not this harness's gap

The morning's two Japanese figures were both FuguMT's. This asks the same question of the checkpoint
already installed, and the answer changes what the choice between them is about.
`docs/UNPROVEN.md` § *The shipped translator on Japanese* has the caveats.

`scripts/measure-translation.py --variant runs/translation-onnx/fp32-merged --languages ja
--num-beams 6`, target token at its default `>>eng<<`, FLEURS `ja_jp` test, all 321 distinct
sentences, all 321 shared with `en_us`, laptop CPU, harness at `28eebd9`.

**chrF++ 46.47 against a source-copy floor of 0.63 — margin +45.84 where +44.37 was required, and
the gate passes**, with zero collapses and three trailing punctuation runs. Japanese is not one of
the 24 languages the 2026-08-20 gate run scored: `opus-mt-tc-bible-big-mul-deu_eng_nld` carries
`mul` on its source side, so this is a 25th language measured separately and **the 23-of-24 result
is untouched**.

**Beside FuguMT's 52.53 on identical footing** — same corpus, same 321 sentences, same floor, same
required margin, beam 6, same signature, same machine, differing only in variant and target token —
**the dedicated checkpoint leads by 6.06 chrF++, and both pass.**

**That is not the gap the survey acted on.** FuguMT was chosen against Helsinki's own leaderboard
BLEU of 4.8 on newstest2021 for this exact checkpoint, set against FuguMT's 23.2 on FLORES-200
devtest — a ratio that reads as unusable against usable, and the reason the shipped checkpoint was
never tried on Japanese. **Neither published figure is wrong and neither is comparable to this
one**: three corpora, two metrics, and a 6-point chrF++ gap that cannot be set beside an 18-point
BLEU gap. What is established is this project's own and is narrower — on FLEURS `ja_jp` at beam 6 on
the CPU, the shipped translator meets the translation gate's criterion one on a Japanese row and
FuguMT scores 6.06 above it. **There is no Japanese gate**: no criterion has been ratified for this
language, and the bar applied here is the 2026-08-20 one computed against Japanese's own floor.

**What it changes.** The 2026-09-04 scoping decision recorded that the shipped checkpoint "would run
today" behind the catalogue's 25-language clamp, on a published figure alone. It now has a measured
one, and the cost side of adding FuguMT is unchanged: 571.1 MiB hosted nowhere, the catalogue's
first CC-BY-SA entry with `docs/LICENSING.md` still unasked, and a compile-time target token that a
single-direction checkpoint cannot use. **Nothing here decides between them** — it prices the
choice, and the decision is the maintainer's.
...
### Decided and built 2026-09-04, night — Japanese translation ships on FuguMT: four decisions, a second translation entry, and the token that stopped being a constant

**The decisions, the maintainer's, on the evening's like-for-like figures.** (1) CC BY-SA 4.0 is
acceptable in the catalogue: the first ShareAlike entry. (2) FuguMT ships in v1.0 as a second
translation entry rather than the shipped checkpoint behind a lifted clamp — the 6.06 chrF++ is
real, the shipped checkpoint's Japanese was the narrowest gate pass of the 25 languages measured, at
+1.47 over the bar with three stuck-punctuation runs in 321 sentences, and that headroom is smaller
than every cascade penalty this project has measured, where FuguMT's own cascade figure, 48.46,
stays above the bar. (3) The translator is chosen by the selected recogniser, with no picker — the
third time the no-spoken-language-picker answer has come out that way. (4) The export is published
from this machine under `jkkma`.

**What was found first: the clamp was prose.** Before anything was built, one run settled whether
Japanese already translated. `uindosill translate` over three Japanese lines through the shipped
checkpoint returned correct English with no change (`runs/20260904-223049-ja-translate-cli-cpu/`),
and a read of every consumer showed `SourceLanguages` assigned in two places and read by nothing:
the "25-language clamp" the evening's entries described was the catalogue's language list and the
sentences around it, not a gate. The compile-time target token was the one real obstacle, and the
UNPROVEN sentence that said the clamp stood in the way is annotated rather than deleted.

**What the build changed.**

- **The target token is a per-checkpoint declaration.** `models.json` declares `targetToken` on the
  many-to-one entry and none on the Japanese one; the sidecar reports at load whether the loaded
  vocabulary holds `>>eng<<` as one piece; the host refuses a disagreement
  (`SidecarTranscriptTranslator.ApplyTargetToken`) before a source is built; and
  `TranslationRequest.Build` marks every source or none, with null a declaration and blank still
  refused. `TranslatorCapabilities.TargetToken` is nullable and required, so every translator states
  one or the other.
- **The decode check is per checkpoint.** The engine held `bad_words_ids` and `forced_eos_token_id`
  to the shipped checkpoint's numbers, which refused every other checkpoint for having a different
  vocabulary; it now holds them to the checkpoint's own `config.json` — decoding starts on pad, pad
  is banned, the end token is forced — which both checkpoints satisfy and a lost
  `generation_config.json` does not.
- **The parity fixture is per checkpoint**, chosen by vocabulary size because the exports write no
  name into `config.json`; a checkpoint with no fixture now warns rather than loading in silence, a
  change the night's own slip forced (`docs/UNPROVEN.md` § *Japanese translation ships on FuguMT*),
  and the two packaging scripts require the Japanese fixture as they require the first.
- **Selection by recogniser.** `ModelCatalog.TranslationModelsFor` returns the entries whose language
  list covers a recogniser's. `transcribe --translate` and the window use it; `translate`, text with
  no recogniser behind it, falls back to the only entry installed and asks for `--model` with two;
  a recogniser that declares no languages — a sideloaded one — is refused rather than guessed for.
  The window re-asks whenever the Models tab's highlight moves or the session loads a model.
- **The catalogue.** `fugumt-ja-en-fp32`: nine files, 360.8 MiB, pinned by size and digest at commit
  `9e075eec` of `jkkma/fugumt-ja-en-onnx` and verified on install. The many-to-one entry is renamed
  *European to English translation* beside *Japanese to English translation*, the morning's naming
  decision applied to the second task that has two entries.
- **The licence.** `CcBySaAttribution` — CC BY's seven elements and a required ShareAlike statement —
  in `Attribution.cs`, `NOTICE.md`, the export's card and `docs/LICENSING.md`, which records what was
  read at the pinned revision (no LICENSE, no NOTICE, no copyright notice, a creator designated only
  by pseudonym) and this project's reading of what ShareAlike reaches. No lawyer has read it.
- **Ten tests**, 1673 passing and 9 skipped, and the export script's smoke decodes with
  `renormalize_logits` as the sidecar does, so it compares two decodes rather than two collapses.

**What it does not settle** is in `docs/UNPROVEN.md` § *Japanese translation ships on FuguMT*: no
person has clicked through the window from here, no person has rated the English, no timing is a
figure, and the desktop owes the CUDA arm.

### Polished 2026-09-06 — the window rendered page by page, and what only a picture could show

**The window was drawn with a renderer, every page of it, for the first time since the interface
was designed.** The suite draws the same window and measures it, but the headless host it draws on
has no renderer: a hairline, a grey and a glyph are exactly what it cannot see, and it has been the
...
- **The speaker chips were two sizes.** Two of the eight treatments are outlines, and the one-unit
  edge only they carried made them two units wider and taller than the chips beside them: the
  strip that names the voices stood at two heights, and every line spoken by an outlined speaker
  began two units to the right of its neighbours. Every chip now carries the edge, transparent on
  the filled ones. `SpeakerChipGeometryTests` holds the three treatments to one height on the strip
  and one word offset in the cues.
- **The scroll bars were the toolkit's greys.** A `#7A7A7A` thumb, three units wide down the edge
  of every pane that scrolls, on an `#E6E6E6` track: the only two greys in the window the palette
  did not know. The thumb takes the disabled-text grey at rest. The first override was a `Color`
  key that exists in the assembly and is folded into a brush at theme load, so it changed nothing
  — read as changed by eye, caught by `ScrollBarTests` reading the brush off the thumb; gotcha 45.
- **The halves of Settings were switched by Fluent's tab strip** — two words at 20px over a rounded
  underline, the one header in the product not drawn in the design's terms. The pill switcher the
  headerbar and the About window use now stands over a headless `TabControl`, bound to that
  control's own index rather than a view model property, because which half is open is not state
  anything else reads.
- **Think before answering was a ToggleSwitch**, drawn with its label above the track and an "Off"
  beside it: the one rounded control on a page of square boxes. It is a box.
- **The answer's sources sat in a Fluent Expander**: a bordered box the height of a button, with a
  chevron out of an icon font the window does not ship. A chrome-less disclosure toggle with a
  drawn chevron replaces it, and its presenter is deliberately not named `PART_ContentPresenter` —
  the ToggleButton theme paints a presenter of that name in the accent when checked, and found
  this one by name inside a template it did not write.
- **A disabled chip lost its ground.** The window-wide rule paints a disabled button's presenter
  white inside a grey hairline, which on a pill left a bare grey glyph: the "?" of a claim the
  model could not anchor, and every citation chip while the transport is dead. A disabled chip
  keeps its ground and dims, at the opacity a provisional line already uses.
- **A disabled text box filled solid grey**, and the find box on the Ask tab is disabled for the
  whole of a first visit. It dims and keeps its ground, and so does a hovered one, on the design's
  own words about a hairline that does not change the box. `FieldStateTests` reads both off the
  template's border.
- **The Models notes ran under the scroll bar** to the column's last unit, where "word" arrived as
  "wor". Sixteen units of right padding.
- **The transcript area said nothing while empty** — on a first launch the largest thing in the
  window and the only one saying nothing, where the Ask tab has explained its empty pane since it
...
**Fourteen tests, 1873 passed and 9 skipped of 1882 tests.** Ten of them read the rendered brush or
...
under the strips that ask for the passes; at the window's smallest size it has no height at all,
and the status line under it goes with it. That is the 2026-08-23 placement doing what it was placed
to do rather than a hairline to straighten, and each way out is a decision — the strips beside the
queue under the Start button they configure, a drop zone that shrinks once the queue has files, or
...
**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** The line counts
...
fonts rather than on copy is not the check wanted. The suite is unchanged: 1882 tests, nine skipped,
...
reach it. The suite is 1882 tests, nine skipped, all green.
...
### Changed 2026-09-07, evening — the bundled Python ships as one file, because 55,256 of them cost ten minutes an update

**An rc.14 → rc.15 in-app update was watched end to end on the desktop, and it took 10m 05s.** The
release moves three C# files of settings-page wording. Three seconds of the ten minutes was the
download: the `win-cuda` delta arrived complete at 31,799,583 bytes, exactly what
`releases.win-cuda.json` promises. Everything after that was local, and `Update.exe` was sampled
with **zero TCP connections open** while its progress bar was still moving. `docs/UNPROVEN.md`
carries the full timing table.

**The cost was file count, and the package's own composition is what proves it.** The installed
`current/` holds 55,335 files, of which the Python bundle is **55,256** at 1.5 GB and the CUDA and
Vulkan native stack is **55** at 2.0 GB. The larger thing, by half a gigabyte, is a thousandth of
the file count and contributed nothing measurable. Velopack pays for every entry four times — into
the delta-rebuilt package, back out of it, into the install directory, and out of the temporary tree
it drains afterwards — and one 8-second sample of the last phase showed **929,418 filesystem
metadata operations against zero bytes read and zero written**, about 42 per file, which is what a
real-time scanner in the path looks like.

**So the publish carries `python-bundle.zip` and `python-bundle.json`, and the bundle is unpacked on
the machine.** `PythonBundleInstaller` stages it, checks that what came out is a whole bundle rather
than half of one, and moves it into `%LOCALAPPDATA%\Uindosill\python\<archive digest>`.

Four decisions inside that, each of which could have gone the other way:

- **Into the user data directory, not the install tree.** The install tree is what Velopack rewrites
  on every update; the user data directory is the one the weights already survive in, for the reason
  `UserDataPaths` gives at length. Unpacking into `current/` would have kept the whole cost and
  moved it somewhere less visible.
- **Named for the archive's digest.** Keying on the *application's* version would have re-unpacked
  55,256 files on every release, which is the same bill in a different envelope. **The digest does
  less than it was meant to, and that was measured the same evening** — see the correction below.
- **Unpacked lazily, on the first speaker-labelling or translation request.** Doing it during the
  update puts the minutes back where they were. Doing it at launch charges every user for two
  opt-ins most never run — core transcription is parakeet.cpp and never touches the sidecar. The
  hook is the sidecar's own default factory, so it fires exactly when something first needs a
  child, inside whatever job asked for it.
- **One archive, not two.** The installer's copy and the separate `uindosill-python-win-x64.zip`
  download were two zips of the same tree; they are now the same file, copied. Two artefacts that
  can disagree is one more than this needs, and the one a user hand-unpacks would have been the one
  nobody checked against an install.

**The prune is deliberately narrow.** A superseded unpack is 1.5 GB in a directory that survives
uninstall, so it has to be removed — but that directory is also where a user is invited to unpack
the download by hand. So it deletes only names that are 64 lowercase hex characters, which is a
shape nobody chooses by accident, and a test stages a hand-unpacked bundle beside a stale one to
hold it.

**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Nine new, all on
the unpack: where it lands and what provenance it reports, that a second call does not run again
(asserted by a marker file surviving, because what a user would feel is the tree being rewritten),
that a loose bundle beside the application still wins and suppresses the unpack entirely, that a
superseded digest-named directory is removed and a hand-unpacked one is not, that a truncated
archive and a path-traversing entry are both refused with nothing left behind, and that a build
carrying no archive fails exactly as it did before.

**Packed the same evening, and the headline is measured.** `package-windows.ps1 -Version
1.0.0-rc.16 -Channels win-cuda -SkipVendor` produced a full package of **87 entries against
rc.15's 55,342** — a 636-fold drop, against a prediction of "about 90". Its uncompressed total fell
from 3.40 GiB to 2.51 GiB as well, because the bundle now travels as one already-compressed member
instead of 1.5 GB of loose files, so the extraction into `current/` writes less as well as touching
fewer things. Package 1,962,713,193 bytes, Setup.exe 1,967,183,465, `python-bundle.zip` 459,567,606
carrying 55,256 files, and the read-back found the natives, the ask engine, the companions and the
weights all where the channel promises them.

**Two faults reached that run, and both are the kind only packing finds.**

- **The archive's root was `python-bundle/`, not `python/`.** `ZipFile.CreateFromDirectory` with
  `includeBaseDirectory` names the root after the directory it is handed, and the staging directory
  had been put at `packaging/python-bundle`. The result would have been an archive neither the
  unpack nor a hand-unpack could resolve. **The script's own read-back caught it** — the check was
  written into this change and refused the run, which is the whole argument for reading artefacts
  back rather than trusting that they assembled. Staging is now `packaging/python-bundle/python`.
- **`unpackedBytes` was rendered `1397504487.0`, and that would have shipped.** PowerShell's
  `Measure-Object -Sum` returns a Double and `ConvertTo-Json` writes it with a decimal point;
  `System.Text.Json` throws rather than truncating into a `long`; and `TryReadArchiveManifest`
  turned the throw into `null`, which on an installed copy is indistinguishable from a build that
  carries no archive at all. **Every installed rc.16 would have had speaker labelling and
  translation dead, blaming a download the user already had.** Nothing in the suite or the build
  could see it — it was found by deserialising a really-packed manifest. Fixed at the source with a
  `[long]` cast, guarded by a read-back that asserts those three fields render as plain integers in
  the file's *text* (the failure is a rendering one, so the text is where the two ends meet), and
  given a throwing `ReadArchiveManifest` on the unpack path so "missing" and "malformed" no longer
  collapse into one silent answer. Two tests pin the exact literal that shipped.

**And a claim this entry made an hour earlier is now corrected.** The digest naming was supposed to
mean that an update leaving the bundle alone unpacks nothing. **It does not hold across builds.**
Two runs over an identical package set produced archives 350 bytes apart, and so different digests,
while `unpackedBytes` and the entry count were identical to the byte. The cause is the bundle's
**16,469 `.pyc` files** across 2,171 `__pycache__` directories: a `.pyc` header carries its source's
mtime, so a rebuild produces same-length, byte-different files. Every release is a fresh build, so
**the realistic expectation is one unpack per release**, not one per bundle change. The fix is to
key the directory on the bundle's inputs — the CPython pin, `requirements-bundle.lock.txt`, the
...
### Fixed 2026-09-07, night — rc.16 greyed out both opt-ins and offered no way to reach the unpack that would have enabled them

**The caveat the entry above closes with was the defect.** "The unpack check ran from a harness
rather than from the window" was written as a gap in coverage; it was in fact a description of a
product that did not work. The maintainer installed `1.0.0-rc.16` from the release and found
**Translate to English and Label speakers both disabled**, each with a paragraph naming a class and
two absolute paths.

**The window asked the wrong question, and on a fresh install the right answer to it was no.**
`EngineProvider` decided availability with `PythonRuntime.TryResolve`, which asks whether a bundle
is on disk *now*. From rc.16 the bundle arrives as `python-bundle.zip` and is unpacked on first use
— so on every fresh install the resolver correctly says there is no bundle, the window correctly
disables both opt-ins, and the unpack that would have satisfied them **only runs when one of those
opt-ins is used**. Neither could ever happen. The command line was untouched, because it builds a
sidecar rather than asking whether it could; the defect was the window's alone, and every one of
the eleven tests written for the unpack began after the point the window never got past.

`PythonRuntime.CanRun` is the question that should have been asked: whether the feature *can* run,
which a build carrying the archive can. It does not unpack — it is called while drawing a page, and
the unpack writes 1.3 GB — it only declines to call a build unavailable for a state it can leave in
twenty seconds.

**The sentence went too.** It named `PythonBundleInstaller` and two absolute paths at a reader who
could act on neither, four commits after f1e93c4 and 7f4919e spent two rounds taking exactly that
register out of the Settings pages. It is now one sentence saying the thing is unpacked on first use
and takes about half a minute, and a test asserts the message carries no class name, no path and no
file name.

**Verified against the installed copy rather than a fixture**, which is the correction that matters
here: driving the real `%LOCALAPPDATA%\UindosillDesktop\current\python-bundle.zip` into the real
`%LOCALAPPDATA%\Uindosill` showed `TryResolve` false and `CanRun` true on the same machine, then
unpacked 55,256 files in **21.4 s**, resolved the CUDA pack at `…\Uindosill\python-cuda` alongside
the digest-named bundle, and completed the handshake at `protocol 6`. That also settles the last
open item from the entry above — the pack's precedence on real hardware.

**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** Five new: that a
fresh install carrying only the archive still reports it can run, that a build with neither bundle
nor archive cannot and says why, and that the not-yet-unpacked sentence carries no jargon; the older
test asserting that sentence *named* the archive was inverted, since naming it is what made it
developer copy.

**What it does not settle.** The first run after an update now pauses about twenty seconds with
nothing on screen explaining why, because the unpack happens inside the job with no progress of its
own — `PythonBundleProgress` exists and reports, and nothing in the window reads it yet. And rc.16
is published with the defect: anyone installing it meets two disabled checkboxes, and the fix is
in rc.17 or in unpacking the archive by hand.

### Fixed 2026-09-12 — desktop audit shutdown, output collisions and stale controls

The desktop audit at `e4ebaab` reproduced six defects, with its original evidence retained under
`runs/audit-20260912/` on the laptop. Starting transcription now marks the run busy and establishes
cancellation before loading the model. Both Start and Run again check cancellation again after a
native load that cannot abort. Shutdown stops new work, cancels and awaits transcription, link
fetching, export and subtitle muxing before disposing the session. The real downloader and muxer
wait for their child process to exit after requesting termination, before deleting partial files.
Start and Run again refuse to begin while a link fetch is pending, so a late download cannot miss
the batch's queue snapshot.

CLI preflight checks every concrete output filename, including tidied output, translation-failure
fallbacks and compound subtitle extensions. `a.wav` plus `a.tidy.wav` with `--tidy --overwrite`
now returns a usage error before any output is written. The selected recording notifies all
transcript-pane visibility properties, and the model-removal button invalidates its command when
the installed set or transcription activity changes. Its direct execution path also checks the
busy condition. Citation validation permits a fixed 25 ms end discrepancy for a span starting
inside the recording, covering the previously measured AAC container/sample-clock disagreement;
larger overruns remain invalid and no transcript timestamp is rewritten.

**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** The 38 new cases
cover cooperative and uninterruptible loading, cancellation/retry, fetch and mux cleanup, output
collisions, shown headless-window bindings and citation boundaries. Release built with zero
warnings/errors. The rendered-line utility reported no overflowing blocks. Actual-media follow-up
evidence and its limits are recorded in `docs/UNPROVEN.md`; detailed logs and reproduction notes
remain under `runs/audit-fixes-20260912/` and its report folder on the laptop's Drive route.

### Built 2026-09-12 — fuller video answers and complete section summaries

The supplied CSB388 comparison exposed two different evidence gaps: automatic summaries sampled
past short discussions, and a pointed Zelda question retrieved scattered mentions of remakes while
missing the opening discussion's later details. Automatic long summaries now read consecutive
sections and combine generated notes; short summaries and explicit whole-transcript requests keep
their single-pass path. Topic retrieval ranks broader passages and keeps their original fine
windows, including surrounding explanation, inside the configured evidence allowance.

Generated notes remain separate from transcript evidence. Intermediate claims need checked source
citations; synthesis keeps those original references. Intermediate ranges crossing unseen gaps
are rejected; final answers mark unseen citations unresolved, hide their timestamps and disable
seeking without discarding the other claims. This scope check also applies to topic answers.
A failed or cancelled section cannot appear as a completed overview. Stop is checked again after
the final streamed frame, so a late cancellation cannot publish a completed, copyable answer.
Introductory paragraphs now show and copy the same quote caveats as individual bullets. Citation parsing
accepts the model's unambiguous `S45-76` shorthand while rejecting malformed endpoints; validation
checks those bounds against the source. The URL-fetch display title can name the recording without treating its
punctuation as a path or extension.

The first full-length attempt exposed another failure: final synthesis broadened source ranges
across gaps, discarding the completed overview under an overly strict final check. Final citations
now follow the existing check-and-mark policy above. A separate draft-review pass repeated the
same unsupported inference and was removed. Constrained quoting hit the token cap on the tested
checkpoint, so grammar remains opt-in; an explicitly constrained intermediate pass forbids uncited
markers. Thinking mode exposed a separate budget defect: the native server was never given the
thinking allowance and could spend the entire response cap before answering. It now receives
`--reasoning-budget` when thinking is enabled; the overall response cap remains unchanged.

A subsequent complete-source attempt reached final synthesis but exhausted the retrieval-sized
1,024-token answer allowance. The app correctly rejected the cut-off output. Final synthesis now
has its own configurable `MaxSummaryTokens` allowance, defaulting to 2,048; other stages retain
`MaxAnswerTokens`. Its overview prompt asks for short topic bullets across the beginning, middle
and end rather than retelling each section note. Generated section notes can still contain
semantic errors; citation checks do not judge whether their paraphrases are true.

**1882 tests, no weights, no display, no network — 1873 passed and 9 skipped.** The 97 new cases
cover passage context, budget failures, complete partitioning, generated-note provenance, citation
scope, final-frame cancellation, lead quote warnings, and prompt/token-budget contracts. The four separately gated native-engine checks also
passed on the CPU with the installed Gemma E4B checkpoint. The Gemma 4 12B comparison and its
remaining quality and latency limits are recorded in `docs/UNPROVEN.md`; evidence stays under
`runs/gemma4-youtube-20260912/` and its laptop run-report folder.
